...
public class SystemTray : IDisposable
{
    ...
    public void SetTrayIcon(string resPath)
    {
        ...
        {
            var icon =Icon.ExtractAssociatedIcon(resPath);//ÓÐ¼¸ÂÊ»ñÈ¡²»µ½£¿
            
            //Debug.Log($"{nameof(SystemTray)}: use exe icon {icon is null}");
            var bmp = icon.ToBitmap();
            Debug.Log(Path.Combine(/*"..",Globals.BinariesFolder.Replace('/','\\'),*/"11.bmp"));
            bmp.Save(Path.Combine(/*Globals.BinariesFolder.Replace('/','\\'),*/"11.bmp"), ImageFormat.Bmp );
            trayIcon.Icon = icon;
        }
        else
        {
